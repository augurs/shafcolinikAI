...
async def upload_images(
    ...
    # Handle height input
    try:
        user_height = float(height_cm) if height_cm else DEFAULT_HEIGHT_CM
    except ValueError:
        user_height = DEFAULT_HEIGHT_CM

    # Read uploaded files once and store in memory
    received_files: Dict[str, UploadFile] = {"front": front}
    if left_side:
        received_files["left_side"] = left_side

    file_bytes_map: Dict[str, bytes] = {}
    for pose_name, upload_file in received_files.items():
        image_bytes = await upload_file.read()
        if not image_bytes:
            raise HTTPException(status_code=400, detail=f"{pose_name} image is empty or could not be read.")
        file_bytes_map[pose_name] = image_bytes

    # Validate front image first
    front_image_np = np.frombuffer(file_bytes_map["front"], np.uint8)
    front_image = cv2.imdecode(front_image_np, cv2.IMREAD_COLOR)
    if front_image is None:
        raise HTTPException(status_code=400, detail="Front image is corrupted or unreadable.")
    ...
    for pose_name, image_bytes in file_bytes_map.items():
        image_np = np.frombuffer(image_bytes, np.uint8)
        frame = cv2.imdecode(image_np, cv2.IMREAD_COLOR)
        if frame is None:
            raise HTTPException(status_code=400, detail=f"{pose_name} image is corrupted or unreadable.")
    ...
    debug_info = {
        ...
        "user_height_cm": float(user_height)
    }
